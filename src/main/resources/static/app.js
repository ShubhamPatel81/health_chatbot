    ...
    const HeaderGT = () => (
        <header className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 py-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h1 className="text-xl font-bold">Arogya Health Care</h1>
    ...
    const FooterGT = () => (
        <footer className="bg-gray-100 p-6 py-4 text-center text-sm text-gray-600 border-t border-gray-200">
            <p className="flex items-center justify-center">
                <span>© 2025  Arogya Health Care</span>
            </p>
        </footer>
    );
    ...
    const handleSend = async () => {
      try {
        ...
        console.log(res.data); // Show response
        setMessage("");
      } catch (err) {
        console.error(err);
      }
 ...
 if (trimmedInput.startsWith('/book')) {
     const lowerInput = trimmedInput.toLowerCase();

     let doctorType = '';
     let fees = '';

     if (lowerInput.includes('dermatologist')) {
         doctorType = 'Dermatologist';
         fees = '₹250';
     } else if (lowerInput.includes('cardiologist')) {
         doctorType = 'Cardiologist';
         fees = '₹500';
     } else if (lowerInput.includes('neurologist')) {
         doctorType = 'Neurologist';
         fees = '₹600';
     } else if (lowerInput.includes('dentist')) {
         doctorType = 'Dentist';
         fees = '₹300';
     } else if (lowerInput.includes('general') || lowerInput.includes('physician')) {
     ...
     iif (doctorType) {
        setMessages((prev) => [
          ...prev,
          { text: `You have selected a ${doctorType}. The consultation fee is ${fees}. Please fill out the form below:`, isUser: false },
          { isUser: false, isIframe: true, iframeUrl: 'http://localhost:9090/book' },
        ]);
      }
        else {
         // Redirect even if doctor not mentioned
         setMessages((prev) => [
             ...prev,
             { text: `Certainly, I'd be happy to help you book an appointment. Please specify the doctor and your preferred date & time. Redirecting you to the booking form...`, isUser: false },
         ]);
         window.open('http://localhost:9090/book', '_blank');
     }
     return; // Don't let it fall through to other logic
 }
